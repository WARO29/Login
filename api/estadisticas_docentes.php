...
$docenteModel = new models\DocenteModel();
$representanteModel = new models\RepresentanteDocenteModel();

// Obtener estadísticas de votación de docentes
$estadisticasVotacion = $votosModel->getEstadisticasVotacionDocentes();

// Obtener el conteo de votos por representante
$votosRepresentantes = $votosModel->getConteoVotosRepresentantesDocentes();

// Obtener los votos recientes de docentes (últimos 10)
...
if (is_array($votosRecientes)) {
    foreach ($votosRecientes as $voto) {
        ...
        // Asegurar que el nombre del docente esté presente
        if (!isset($voto['nombre_docente']) || empty($voto['nombre_docente'])) {
            // Intentar obtener el nombre del docente desde la base de datos
            if (isset($voto['id_docente'])) {
                $docente = $docenteModel->getDocentePorDocumento($voto['id_docente']);
                if ($docente && isset($docente['nombre'])) {
                    $votoFormateado['nombre_docente'] = $docente['nombre'];
                } else {
                    $votoFormateado['nombre_docente'] = 'Docente';
                }
            } else {
                $votoFormateado['nombre_docente'] = 'Docente';
            }
...
error_log('Estadísticas de votación: ' . json_encode($estadisticasVotacion));
error_log('Votos por representante: ' . json_encode($votosRepresentantes));
error_log('Votos recientes: ' . json_encode($votosRecientes));

// Adaptar los nombres de los campos para que coincidan con los esperados en el JavaScript
$estadisticasAdaptadas = [
    'total_docentes' => $estadisticasVotacion['totalDocentes'] ?? 0,
...
$response = [
    ...
    'votosRepresentantes' => $votosRepresentantes,
